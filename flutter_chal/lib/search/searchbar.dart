...
class _searchbarState extends State<searchbar> {
  ...
  Future<Map> fetchMovies(String searchTerm, int page) async {
    final response = await http.get(
        "https://api.themoviedb.org/4/search/movie?api_key=6866b6e81ff7327379e4b8d14ba50af1&language=pt-BR&query=" +
            searchTerm +
            "&page=" +
            page.toString() +
            "&include_adult=false");
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
        theme: ThemeData(appBarTheme: AppBarTheme(color: Colors.brown)),
        home: Scaffold(
            appBar: AppBar(
              leading: IconButton(
